...
You will also need SpiderMonkey ESR 140 installed where Meson can find
it.
Generally this means that the `mozjs-140.pc` file needs to be installed
in a location known to pkg-config, and the `libmozjs-140.so` file needs
to be in the path for loading libraries.

Many Linux distributions have development packages for SpiderMonkey 140
